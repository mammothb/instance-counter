...
def display_instances(image, boxes, masks, class_ids, class_names, scores=None):
    ...
    for i in range(N):
        ...
        # Mask
        mask = masks[:, :, i]
        im_copy = apply_mask(im_copy, mask, c)
        im_copy = cv2.rectangle(im_copy, (x1, y1), (x2, y2), c, 2)
        im_copy = cv2.putText(
            im_copy, caption, (x1, y1), cv2.FONT_HERSHEY_COMPLEX, 0.7, c, 2
        )

    im_copy = cv2.putText(
        im_copy, str(counter), (0, 20), cv2.FONT_HERSHEY_COMPLEX, 0.7, c, 2
    )
    return im_copy
